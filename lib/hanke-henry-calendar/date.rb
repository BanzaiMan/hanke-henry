...
145733
  ...
  def self.hh(*args)
    hh_year, hh_month, hh_day, hour, minute = _validate(*args)
    Date.jd _to_julian_date(hh_year, hh_month, hh_day)
  end
  ...
  def self._validate(*args)
    if args.length > 4
      raise ArgumentError, "Wrong number of arguments (#{args.length} for 0..8)"
    end
